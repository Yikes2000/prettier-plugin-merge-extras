 ...
 */
export function alignObjectProperties(code: string, options: any): string {
    opts = options;
...
const RE_START_IS_CLOSE_CURLY = /^[ \t]*\}/;
const RE_START_IS_CLOSE_SYMBOL = /^[ \t]*[})\]]/;

// Test if a segment ends with open brace '{', plus optional '//' or '// ... //'
const RE_END_IS_OPEN_CURLY = /\{(?:[ \t]*\/\/(?:[^\n]*\/\/)?)?\n[ \t\n]*$/;

// Detect if a segment ends with open curly but EOL '//', indicating prettier-ignore
const RE_END_IS_EOL_IGNORE = /\{(?:[ \t]*\/\/(?:[^\n]*\/\/)?)\n[ \t\n]*$/;

// Detect open brace from function declaration, e.g. ') {' or '=> {', and exceptions
const RE_END_IS_FUNC_OPEN_BRACE = /[\w)>]\s+\{[ \t\n]*$/;
const RE_END_IS_OBJ_OPEN_BRACE = /\b(?:return|throw|interface \w+)\s+\{[^\n]*[ \t\n]*$/i;
